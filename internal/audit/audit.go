...
func (s *Store) Summary(ctx context.Context, filter Filter) (Summary, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT
		...
		COALESCE(SUM(CASE WHEN result != 'success' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN action = 'validation.request' AND result = 'success' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN action = 'validation.request' AND result != 'success' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN action = 'mail.send' AND result = 'success' THEN 1 ELSE 0 END), 0),
		COALESCE(SUM(CASE WHEN action = 'mail.send' AND result != 'success' THEN 1 ELSE 0 END), 0),
		...
		&summary.ValidationSuccesses, &summary.ValidationFailures, &summary.MailSendSuccesses, &summary.MailSendFailures, &summary.Count4xx, &summary.Count5xx)
	if err != nil {
		return Summary{}, err
	}
	return summary, nil
...
func isSensitiveKey(key string) bool {
	...
	sensitiveParts := []string{"password", "passwd", "secret", "token", "authorization", "cookie", "body", "payload", "message", "smtp"}
...
func whereClause(filter Filter) (string, []any) {
	...
	if filter.Action == publicAPIActionFilter {
		clauses = append(clauses, "action IN ('public.api.access', 'validation.request', 'mail.send')")
	} else {
